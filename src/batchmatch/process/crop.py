...
def _normalize_crop_outputs(outputs: Optional[Sequence[str]]) -> Optional[set[str]]:
    ...
    if "all" in normalized:
        return set(_CROP_OUTPUTS)
    unknown = normalized - _CROP_OUTPUTS
    if unknown:
        raise ValueError(f"Unknown outputs: {sorted(unknown)}. Valid: {sorted(_CROP_OUTPUTS)}")
    return normalized
...
class RandomCropStage(Stage):
    ...
    def __init__(
        ...
        *,
        ...
        self._gx_key = (
            gx_key if gx_key is not None else (self.DEFAULT_GX_KEY if crop_gx else None)
        )
        self._gy_key = (
            gy_key if gy_key is not None else (self.DEFAULT_GY_KEY if crop_gy else None)
        )
        self._box_key = (
            box_key if box_key is not None else (self.DEFAULT_BOX_KEY if adjust_box else None)
        )
        self._quad_key = (
            quad_key if quad_key is not None else (self.DEFAULT_QUAD_KEY if adjust_quad else None)
        )
        self._points_key = (
            points_key
    ...
    def _randint(self, low: int, high: int) -> int:
        value = torch.randint(low, high + 1, (1,), generator=self._generator).item()
        return int(value)
    ...
    def forward(
        ...
        *rest: ImageDetail,
        ...
        for i, td in enumerate(image_details):
            ...
            if td.H != H or td.W != W:
                raise ValueError(
                    "All inputs must have same spatial dimensions. "
        ...
        for b in range(B):
            ...
            combined_mask = compute_intersection_mask(masks_at_b)

            if combined_mask.sum() == 0:
                if self._allow_full_image:
                    combined_mask = torch.ones_like(combined_mask)
                else:
                    raise ValueError(
            ...
            domain_box[2] = domain_box[2] + 1
            domain_box[3] = domain_box[3] + 1
            ...
            cx0 = self._randint(min_x0, max_x0)
            cy0 = self._randint(min_y0, max_y0)
...
class CropStageBase(Stage):
    ...
    def _invalidate_warp_data(self, warp) -> None:
        if warp is None:
            return
        ...
        *rest: ImageDetail,
        ...
        for b in range(B):
            ...
            for td in image_details:
                mask = td.get(mask_key, None)
                if mask is not None:
                    masks_at_b.append(mask[b : b + 1])  # (1, 1, H, W)
                else:
                    masks_at_b.append(torch.ones(1, 1, H, W, device=td.image.device))

            combined_mask = self._combine_masks(masks_at_b)  # (1, 1, H, W)
            ...
            crop_box = mask_to_box(combined_mask).squeeze(0).squeeze(0)  # (4,)

            crop_box = crop_box.clone()
            crop_box[2] = crop_box[2] + 1  
            crop_box[3] = crop_box[3] + 1
            ...
            for td in image_details:
                for key in [self._image_key, self._mask_key, self._window_key, self._gx_key, self._gy_key]:
                    ...
                    if tensor is None:
                        continue
                    cropped = crop_spatial_to_box(tensor[b : b + 1], crop_box)
                    if key not in cropped_data:
                        cropped_data[key] = []
                    cropped_data[key].append(cropped)
                ...
                if self._box_key is not None:
                    ...
                    if box is not None:
                        ...
                        if self._box_key not in cropped_data:
                            cropped_data[self._box_key] = []
                        cropped_data[self._box_key].append(adjusted)
                ...
                if self._quad_key is not None:
                    ...
                    if quad is not None:
                        ...
                        if self._quad_key not in cropped_data:
                            cropped_data[self._quad_key] = []
                        cropped_data[self._quad_key].append(adjusted)
                ...
                if self._points_key is not None:
                    ...
                    if points is not None:
                        ...
                        if self._points_key not in cropped_data:
                            cropped_data[self._points_key] = []
                        cropped_data[self._points_key].append(adjusted)

            out_data = {}
            for key, tensors in cropped_data.items():
                if len(tensors) == N:
                    out_data[key] = torch.cat(tensors, dim=0)  # (N, ...)

            out_td = ImageDetail(out_data, batch_size=[N])

            if self._invalidate_warp:
                warp = out_td.get(ImageDetail.Keys.WARP.ROOT, None)
                self._invalidate_warp_data(warp)
